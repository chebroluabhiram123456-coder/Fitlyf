...
class _AddExerciseScreenState extends State<AddExerciseScreen> {
  ...
  void initState() {
    ...
    if (_isEditMode) {
      ...
      if (ex.imageUrl != null && ex.imageUrl!.isNotEmpty) _imageFile = File(ex.imageUrl!);
      if (ex.videoUrl != null && ex.videoUrl!.isNotEmpty) _videoFile = File(ex.videoUrl!);
    }
  }
  ...
  void _saveForm() {
    ...
    if (!isValid) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please fill out all required fields.'), backgroundColor: Colors.redAccent),
      );
      return;
    }
